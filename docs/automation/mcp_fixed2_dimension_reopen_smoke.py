"""Synthetic L2 proof that a fixed-decimal dimension survives linked edits.
...
def main() -> None:
    ...
    report = {"schema_version": "mcp-fixed2-dimension-reopen-l2-2", "status": "failed",
              "dimension_style": "OCS_ASSOC_FIXED2",
    ...
    try:
        ...
        created = client.tool("ocs_execute", {"ocs_session_id": session,
            "request": {"op": "run_script", "request_id": "assoc-create-" + uuid.uuid4().hex,
                        "strict": True, "commands": [
                            "SETVAR INSUNITS 6",
                            "DIMSTYLE NEW OCS_ASSOC_FIXED2",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimtxt 0.25",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimasz 0.08",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimgap 0.02",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimscale 1",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimlfac 1",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimdec 2",
                            "DIMSTYLE SET OCS_ASSOC_FIXED2 dimzin 0",
                            "CDIMSTY OCS_ASSOC_FIXED2", "DIMASSOC 2",
                            "LINE 70,0 72.5,0",
                            "DIMLINEAR 70,0 72.5,0 71.25,1"]}})
        handles = [change["handle"] for change in created.get("changes", [])
                   if change.get("kind") == "Added"]
        if created.get("successful_commands") != 13 or len(handles) != 2:
