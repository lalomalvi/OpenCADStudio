...
from apartment_south_window_compiler import compose
from south_window_observation import score
...
def assess(run, image, binary, external_path):
    base = run.parent / "apartment-integrated-two-windows-v1"
    observed = run.parent / "apartment-south-window-v1"
    ...
    if freeze.get("schema_version") != "m7-apartment-three-window-trial-freeze-1" or \
    ...
    if (report.get("schema_version") != "m7-apartment-three-window-trial-1" or
            report.get("status") != "passed_scoped_three_window_l2" or
    ...
    if (external.get("product") != "AutoCAD Core Console" or
            ...
            external.get("model_census_count") != 50 or
            external.get("model_types") != {"LINE": 42, "ARC": 1, "DIMENSION": 7} or
    ...
    return {"schema_version": "m7-apartment-three-window-l4-1",
            "status": "passed_scoped_l4" if count == 50 else "failed_scoped_l4",
            "acceptance_m7": False, "geometry_matches": count, "geometry_total": 50,
