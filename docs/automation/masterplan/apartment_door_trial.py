"""Execute the scoped apartment jamb and door symbol in an owned CAD process."""
...
from apartment_door_compiler import compose, _last_observation
...
def validate_bundle(root: Path, source: Path, binary: Path):
    ...
    if (frozen.get("schema_version") != "m7-apartment-door-trial-freeze-1" or
...
def main() -> None:
    ...
    report = {"schema_version": "m7-apartment-door-trial-1", "status": "failed",
              "acceptance_m7": False,
    ...
    try:
        ...
        invocation = Invocation(
            ...
            "door-" + hashlib.sha256(str(root).encode()).hexdigest()[:24],
        ...
        gui.wait(timeout=60)
        report["status"] = "passed_scoped_door_l2"
