...
def main():
    ...
    parent = root.parent
    base = parent / "apartment-integrated-door-v10b"
    observed = parent / "apartment-north-window-v1"
    ...
    expected = {
        ...
        "compiler_sha256": digest(Path(__file__).with_name("apartment_window_compiler.py")),
        "compiler_v2_sha256": digest(Path(__file__).with_name("apartment_window_compiler_v2.py")),
        "compiler_v11_sha256": digest(Path(__file__).with_name("apartment_window_compiler_v11.py")),
        ...
        "observation_gate_sha256": digest(Path(__file__).with_name("north_window_observation.py")),
        "cad_binary_sha256": digest(binary),
    }
    if (frozen.get("schema_version") != "m7-apartment-window-trial-freeze-3" or
    ...
    report = {"schema_version": "m7-apartment-window-trial-3", "status": "failed",
    ...
    try:
        ...
        invocation = Invocation("development", root.name, 1,
            "window-v11-" + hashlib.sha256(str(root).encode()).hexdigest()[:24],
        ...
        try:
            ...
            pass
        gui.wait(timeout=60)
        report["status"] = "passed_scoped_window_l2"
