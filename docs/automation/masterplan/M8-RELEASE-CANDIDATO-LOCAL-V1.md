...
## Control posterior de revisión del ejecutable

El empaquetador se reforzó después de crear este paquete histórico: al hacer `stage` de un candidato nuevo, ejecuta el binario `--version` y exige que su revisión de 12 dígitos coincida con el prefijo del HEAD Git limpio que declarará el manifiesto. Un ejecutable de `c875d652` no puede etiquetarse como si procediera de un commit posterior. El manifiesto nuevo registra `binary_source_revision`; `verify` comprueba su concordancia si está presente. El paquete `rc-c875-v1` conserva su manifiesto original y volvió a pasar `verify` sin modificación: ZIP SHA `0D81353A5632F104BA78BBF5927EEF4F496BD4E3706F5E5924DBEB6B252E74E5`, manifiesto SHA `C0BBB61495A5D3E1C571530928A3E8DF0F323EE8ACF7EAFC59841C5569A9D026`. Esta comprobación de prefijo detecta binarios obsoletos, pero no sustituye una atestación criptográfica de compilación. Un candidato nuevo requiere build limpio de su HEAD exacto y su propio smoke.
