"""Second frozen door observation after v1 confused a short upper trace with the leaf."""
...
PROMPT = (
    ...
    "una hoja de puerta horizontal LARGA hacia la derecha desde la jamba inferior "
    "y un arco punteado de giro. La hoja tiene longitud parecida a la altura "
    "de la abertura. Hay un trazo horizontal CORTO cerca de la jamba superior: "
    "ese trazo NO es la hoja ni su punta. Identifica tres puntos en coordenadas del "
    ...
    "leaf_tip_px es el extremo derecho de la hoja larga inferior, en la misma "
    "altura que lower_hinge_px; NO es un punto del arco ni del trazo superior. "
...
def prepare(root: Path, source: Path, cli: Path) -> dict:
    ...
    freeze = {"schema_version": "m7-northeast-door-freeze-2",
              ...
              "prompt_revision_reason": "v1_leaf_tip_confused_with_short_upper_trace"}
...
def validate(root: Path, source: Path, cli: Path) -> dict:
    ...
    if (freeze.get("schema_version") != "m7-northeast-door-freeze-2"
...
def invoke(root: Path, source: Path, cli: Path) -> dict:
    ...
    intent = {"schema_version": "m7-northeast-door-intent-2",
    ...
    result = {"schema_version": "m7-northeast-door-invocation-2",
...
def score(root: Path, source: Path, cli: Path) -> dict:
    ...
    verdict = {"schema_version": "m7-northeast-door-verdict-2",
