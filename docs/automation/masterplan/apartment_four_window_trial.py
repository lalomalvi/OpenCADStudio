...
def main():
    ...
    parent = root.parent
    base = parent / "apartment-integrated-three-windows-v1"
    observed = parent / "apartment-west-window-v1"
    ...
    expected = {
        ...
        "compiler_sha256": digest(Path(__file__).with_name("apartment_west_window_compiler.py")),
        "extension_sha256": digest(Path(__file__).with_name("window_extension.py")),
        ...
        "observation_gate_sha256": digest(Path(__file__).with_name("west_window_observation.py")),
        "cad_binary_sha256": digest(binary),
    }
    if (frozen.get("schema_version") != "m7-apartment-four-window-trial-freeze-1" or
    ...
    report = {"schema_version": "m7-apartment-four-window-trial-1", "status": "failed",
    ...
    try:
        ...
        invocation = Invocation("development", root.name, 1,
            "four-window-" + hashlib.sha256(str(root).encode()).hexdigest()[:24],
        ...
        try:
            ...
            pass
        gui.wait(timeout=60)
        report["status"] = "passed_scoped_four_window_l2"
