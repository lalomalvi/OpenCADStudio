...
def main():
    ...
    if (frozen.get("schema_version") != "m7-apartment-window-trial-freeze-2" or
    ...
    report = {"schema_version": "m7-apartment-window-trial-2", "status": "failed",
    ...
    try:
        ...
        invocation = Invocation("development", root.name, 1,
            "window-v2-" + hashlib.sha256(str(root).encode()).hexdigest()[:24],
