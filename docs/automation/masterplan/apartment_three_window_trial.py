...
def main():
    ...
    expected = {
        ...
        "compiler_sha256": digest(Path(__file__).with_name("apartment_south_window_compiler.py")),
        ...
        "observation_gate_sha256": digest(Path(__file__).with_name("south_window_observation.py")),
        "cad_binary_sha256": digest(binary),
    }
    if (frozen.get("schema_version") != "m7-apartment-three-window-trial-freeze-1" or
    ...
    report = {"schema_version": "m7-apartment-three-window-trial-1", "status": "failed",
    ...
    try:
        ...
        invocation = Invocation("development", root.name, 1,
            "three-window-" + hashlib.sha256(str(root).encode()).hexdigest()[:24],
        ...
        try:
            ...
            pass
        gui.wait(timeout=60)
        report["status"] = "passed_scoped_three_window_l2"
