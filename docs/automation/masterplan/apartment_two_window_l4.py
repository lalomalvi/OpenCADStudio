...
def assess(run, image, binary, external_path):
    base = run.parent / "apartment-integrated-window-v11b"
    observed = run.parent / "apartment-east-north-window-v1"
    ...
    bound = {"source_sha256": _file_sha(image),
             ...
             "compiler_sha256": _file_sha(code / "apartment_second_window_compiler.py"),
             "runner_sha256": _file_sha(code / "apartment_two_window_trial.py"),
             "planspec_sha256": _file_sha(code / "planspec.py"),
             "schema_sha256": _file_sha(code / "planspec-v11.schema.json"),
             "observation_gate_sha256": _file_sha(code / "east_north_window_observation.py"),
             "cad_binary_sha256": _file_sha(binary)}
    if freeze.get("schema_version") != "m7-apartment-two-window-trial-freeze-1" or \
    ...
    if (report.get("schema_version") != "m7-apartment-two-window-trial-1" or
            report.get("status") != "passed_scoped_two_window_l2" or
    ...
    if (external.get("product") != "AutoCAD Core Console" or
            ...
            external.get("model_census_count") != 45 or
            external.get("model_types") != {"LINE": 37, "ARC": 1, "DIMENSION": 7} or
    ...
    return {"schema_version": "m7-apartment-two-window-l4-1",
            "status": "passed_scoped_l4" if count == 45 else "failed_scoped_l4",
            "acceptance_m7": False, "geometry_matches": count, "geometry_total": 45,
