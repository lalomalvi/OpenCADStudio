...
from apartment_window_compiler_v11 import compose
from north_window_observation import score
...
def assess(run, image, binary, external_path):
    base = run.parent / "apartment-integrated-door-v10b"
    observed = run.parent / "apartment-north-window-v1"
    ...
    bound = {"source_sha256": _file_sha(image),
             ...
             "compiler_sha256": _file_sha(code / "apartment_window_compiler.py"),
             "compiler_v2_sha256": _file_sha(code / "apartment_window_compiler_v2.py"),
             "compiler_v11_sha256": _file_sha(code / "apartment_window_compiler_v11.py"),
             "runner_sha256": _file_sha(code / "apartment_window_trial_v11.py"),
             "planspec_sha256": _file_sha(code / "planspec.py"),
             "schema_sha256": _file_sha(code / "planspec-v11.schema.json"),
             "observation_gate_sha256": _file_sha(code / "north_window_observation.py"),
             "cad_binary_sha256": _file_sha(binary)}
    if freeze.get("schema_version") != "m7-apartment-window-trial-freeze-3" or \
    ...
    if (report.get("schema_version") != "m7-apartment-window-trial-3" or
            report.get("status") != "passed_scoped_window_l2" or
    ...
    if (external.get("product") != "AutoCAD Core Console" or
            ...
            external.get("model_census_count") != 40 or
            external.get("model_types") != {"LINE": 32, "ARC": 1, "DIMENSION": 7} or
    ...
    return {"schema_version": "m7-apartment-window-l4-3",
            "status": "passed_scoped_l4" if count == 40 else "failed_scoped_l4",
            "acceptance_m7": False, "geometry_matches": count, "geometry_total": 40,
