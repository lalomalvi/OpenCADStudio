...
| M6.5 recortes visuales trazables | partial scoped L1/L2 | `region_evidence.py` deriva hasta 12 recortes de un único PNG `drawing_only` passed, verifica hash e identidad de documento/revisiones de su ArtifactRef y guarda manifiesto con rectángulos, hash del reporte padre, refs de cada recorte y revisión humana `pending`. La verificación recompara píxeles del hijo con el rectángulo del padre, además de detectar alteración de fuente/hijo y rechazar repetición. Dos zonas de la captura sintética de puerta L2 se generaron e inspeccionaron sin nueva captura ni cambio del PNG original. Los rectángulos son píxeles elegidos manualmente; falta mapeo CAD→viewport, oráculo visual, muestreo por zonas de cohorte y aceptación L5 |
