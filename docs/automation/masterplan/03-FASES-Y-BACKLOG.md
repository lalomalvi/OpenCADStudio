...
| M6.2 bounds de fuente separados | partial L0/L2 | `source_bounds` calcula en 2D huellas de líneas/círculos/muros por un lado y puntos de referencia/colocación de cotas por otro, sin mezclar la anotación con la arquitectura. Para el fixture exterior, arquitectura `[0,4]×[-0.1,0.1]` m y anotación `[2,4.5]×[-0.1,0.1]` m quedaron en reporte L2. `unresolved` señala extremos de arco de puerta y colocaciones no compiladas; texto, flechas, glifos, símbolos más complejos y extents renderizados permanecen fuera del alcance |
