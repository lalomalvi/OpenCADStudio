...
| M3.1, M3.2, M3.3, M3.5, M3.6 | partial L1/L2 | 84 respuestas únicas reproducen 9,797,952 tokens históricos sin exportar rollout; PNG sintético con hash y revisión del frame codificado por shader coincidente, trace RPC sanitizado, AUDIT/SAVE/VERDICT/ARTIFACTS/SEAL v2 verificados. Checkpoint con presupuesto pasos/tiempo sobrevivió reinicio del cliente MCP. Sin recorte limpio de UI, cuotas tokens/capturas/bytes ni tiempos CAD por fase |
| M4.1, M4.2, M4.4, M4.5, M4.6 | partial L1/L2 | PlanSpec v1 estricto y dry-run determinista de líneas/círculos; L2 sintético produjo 3 entidades y mapa ID→handle/capa. Capas distintas de 0 compilan solo con manifiesto probado para el binario exacto. Cotas nativas siguen unsupported |
...
| M5.1 capas | partial L2 | `LAYER NEW`/`CLAYER` asignaron A-WALL a línea por handle; L2 PlanSpec de seis pasos y manifiesto DWG reabierto conservaron A-WALL:1, 0:2. Estilos, anchos y perfil métrico pendientes |
...
| M5.4 bloques/INSERT | partial L2 | Definición -BLOCK desde línea sintética, tres referencias persistidas; dos INSERT conservaron posición, escala 1/2 y rotación 30°/0 tras reapertura interna. Unidades de inserción, contenido del bloque y motor externo pendientes |
| M5.5 HATCH | partial scoped L2 | HATCH nativo desde punto interno de PLINE cerrada; capa A-HATCH y propiedades conservadas. En fixture aislado, tras reabrir, editar un vértice de la PLINE 24→25 actualizó los bordes de HATCH vinculados al handle; otro guardado/reapertura conservó los bordes. Legibilidad por escala, otros contornos y motor externo pendientes |
...
| M6.6 | partial L4 | AutoCAD Core Console 2025 (25.0.162.0.0) abrió DWG sintético AC1032 de este build: AUDIT 0/0, tres medidas de cota DXF 42 coincidentes (2.5/5.0/3.5), 14 entidades de Model censadas y SHA de entrada intacto. El proceso no salió tras `QUIT` y se terminó por timeout; falta cierre limpio y comparación externa completa de HATCH, bloques, cotas asociativas después de editar, estilos y escala |
