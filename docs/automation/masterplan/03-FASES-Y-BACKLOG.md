...
| M4 puerta/ventana v4 | partial L0 | PlanSpec v4 exige por puerta bisagra `start/end`, lado `left/right` relativo al eje dirigido del muro y giro explícito de 90°; por ventana exige alturas `sill_m` y `head_m` con `0 ≤ sill < head`. El validador rechaza campos cruzados, giro ambiguo, altura invertida y claves faltantes. Fixture sintético de puerta `validated_uncompiled`, `executable=false`; no se creó símbolo CAD. Faltan compilador, oráculo L2/L4, uniones y cotas referenciadas |
